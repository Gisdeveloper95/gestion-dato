#!/usr/bin/env python3
"""
Script_TRANSVERSAL_Linux.py - VERSION LINUX NATIVA
============================================
...
FUNCIONALIDADES:
1. Exploracion recursiva de directorios TRANSVERSAL
2. Tabla directorios_transv = directorios
3. Tabla archivos_transv = archivos
...
ESQUEMA DB:
- directorios_transv: cod_dir_transv, ruta_completa, nombre_directorio
- archivos_transv: id_archivo_transv, cod_dir_transv, path_file, peso_memoria, fecha_disposicion
"""
...
DIRECTORIO_LOGS = "/var/log/script_transversal"
...
def configurar_sistema_logs(exportar_logs=False, directorio_logs="/var/log/script_transversal", nivel_detalle="INFO"):
    ...
    try:
        ...
        logger = logging.getLogger('ScriptTransversalLinux')
        ...
        nombre_archivo = f"script_transversal_linux_{timestamp}.log"
        ...
        logger.info("SCRIPT_TRANSVERSAL_LINUX.py - INICIO")
...
def verificar_tablas_db():
    try:
        ...
        cursor.execute("""
            ...
            WHERE table_name IN ('directorio_transversal', 'archivos_transversal')
        """)
        
        tablas = [row[0] for row in cursor.fetchall()]
        
        if 'directorio_transversal' not in tablas:
            log("Creando tabla directorio_transversal")
            cursor.execute("""
                CREATE TABLE directorio_transversal (
        ...
        if 'archivos_transversal' not in tablas:
            log("Creando tabla archivos_transversal")
            cursor.execute("""
                CREATE TABLE archivos_transversal (
...
def obtener_rutas_transversal(municipio_especifico=None):
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        cursor = conn.cursor()
        ...
        log(f"Rutas TRANSVERSAL encontradas: {len(rutas)}")
        return rutas
...
# NOTIFICACIONES DESHABILITADAS PARA TRANSVERSAL
# Las notificaciones son EXCLUSIVAMENTE para INSUMOS y POST

# CLASE SCANNER TRANSVERSAL
class ScannerArchivosTransversal:
    """Scanner optimizado para TRANSVERSAL Linux - explora directorios recursivamente"""
    ...
    def explorar_directorio_completo(self, ruta_base, cod_municipio):
        ...
        try:
            log(f"Explorando estructura TRANSVERSAL en: {ruta_base}", 1)
    ...
    def obtener_crear_directorio(self, conn, ruta_dir, cod_municipio):
        ...
        try:
            ...
            # Buscar por ruta_completa
            cursor.execute("""
                SELECT cod_dir_transv FROM directorios_transv
            ...
            cursor.execute("""
                INSERT INTO directorios_transv (
                    cod_municipio, ruta_completa, nombre_directorio,
                    activo
                ) VALUES (%s, %s, %s, %s)
                RETURNING cod_dir_transv
            """, (cod_municipio, ruta_dir, nombre_dir, True))

            cod_dir_transv = cursor.fetchone()[0]
            conn.commit()

            incrementar_contador('creadas', cod_municipio, 'directorioes')
            log(f"Nueva directorio creada: {nombre_dir}", 2)

            # Sin notificaciones para TRANSVERSAL
            return cod_dir_transv
    ...
    def procesar_archivos_transversal_batch(self, conn, archivos, id_directorio, cod_municipio):
        ...
        try:
            ...
            for archivo_path in archivos:
                try:
                    ...
                    cursor.execute("""
                        SELECT id_archivo_transv, usuario_windows FROM archivos_transv
                        WHERE path_file = %s
                        LIMIT 1
                    """, (archivo_path,))
                    ...
                    if archivo_existente:
                        ...
                        if not MODO_SIMULACION:
                            cursor.execute("""
                                INSERT INTO archivos_transv (
                                    cod_dir_transv, nombre_archivo, path_file,
                                    extension, peso_memoria, usuario_windows,
                                    fecha_disposicion, activo
                                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                                RETURNING id_archivo_transv
            ...
            conn.rollback()
            log(f"Error en procesar_archivos_transversal_batch: {e}", 1)
    ...
    def eliminar_archivos_no_existentes(self, conn, id_directorio, rutas_fisicas, cod_municipio):
        ...
        try:
            ...
            cursor.execute("""
                SELECT id_archivo_transv, nombre_archivo, path_file
                FROM archivos_transv
                WHERE cod_dir_transv = %s AND activo = TRUE
            """, (id_directorio,))
    ...
    def procesar_municipio_transversal_completo(self, ruta_base, cod_municipio):
        """Procesa un municipio TRANSVERSAL completo"""
        try:
            log(f"\n{'='*60}", 1)
            log(f"PROCESANDO MUNICIPIO TRANSVERSAL: {cod_municipio}", 1)
            ...
            for ruta_dir, info_dir in estructura['directorios'].items():
                ...
                if archivos_directos:
                    self.procesar_archivos_transversal_batch(
            ...
            log(f"\nMunicipio TRANSVERSAL {cod_municipio} completado en {tiempo_municipio:.2f}s", 1)

        except Exception as e:
            log(f"Error procesando municipio TRANSVERSAL {cod_municipio}: {e}", 1)
            traceback.print_exc()

def main():
    global MODO_VERBOSE, MODO_SIMULACION
    
    parser = argparse.ArgumentParser(description='Scanner TRANSVERSAL Linux')
    ...
    parser.add_argument('--directorio-logs', default='/var/log/script_transversal')
    ...
    print("="*60)
    print("SCRIPT_TRANSVERSAL_LINUX.py - VERSION NATIVA")
    print("="*60)
    ...
    try:
        ...
        log("Obteniendo rutas TRANSVERSAL...")
        rutas = obtener_rutas_transversal(args.municipio)
        
        if not rutas:
            print("No hay rutas configuradas")
            return
        
        print(f"\nProcesando {len(rutas)} municipios TRANSVERSAL")

        # Crear instancia del scanner TRANSVERSAL
        scanner = ScannerArchivosTransversal()

        # Procesar cada municipio
        for id_path, cod_municipio, path_municipio in rutas:
            try:
                scanner.procesar_municipio_transversal_completo(path_municipio, cod_municipio)
            except Exception as e:
        ...
        print("\n" + "="*60)
        print("SCRIPT_TRANSVERSAL_LINUX.py COMPLETADO")
        print("="*60)
        ...
        if args.exportar_logs and logger:
            logger.info("SCRIPT_TRANSVERSAL_LINUX.py - FIN")
