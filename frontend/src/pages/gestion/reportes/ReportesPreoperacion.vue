  ...
  <div class="reportes-preoperacion-page">
    ...
    <div class="page-header">
      <div class="header-content">
        <h1>Reportes de Preoperación</h1>
          ...
          <!-- Departamentos -->
            ...
            <div class="multi-select-container" @click.stop>
                ...
                <div class="dropdown-items">
                    <!-- IMPORTANTE: Aquí está el problema - necesitamos mostrar el contenido del dropdown -->
                    <label 
            ...
            <tbody>
              <tr v-for="municipio in paginatedMunicipios" :key="municipio.cod_municipio">
                ...
                <td class="text-center">
                  <span class="badge badge-info">{{ getInsumoCount(municipio.cod_municipio) }}</span>
                </td>
              </tr>
            </tbody>
          </table>
...
// SOLUCIÓN INTEGRAL PARA TODOS LOS FILTROS EN REPORTES-POSTOPERACION.VUE
const mecanismosGeneralesDisponibles = computed(() => {
  ...
  if (selectedDepartamentos.value.length === 0 && 
    ...
    console.log(`- Sin filtros previos, mostrando todos: ${mecanismosGenerales.value.length}`);
    return mecanismosGenerales.value;
  }
  
  // Filtrar municipios según los filtros aplicados
  let municipiosFiltrados = [...municipiosList.value];
...
const mecanismosDetalleDisponibles = computed(() => {
  ...
  // Si no hay filtros aplicados y no hay mecanismos generales seleccionados, mostrar todos
  if (selectedDepartamentos.value.length === 0 && 
      selectedMunicipios.value.length === 0 && 
      selectedTerritoriales.value.length === 0 &&
      selectedMecanismosGenerales.value.length === 0) {
    console.log(`- Sin filtros previos, mostrando todos: ${mecanismosDetalle.value.length}`);
    return mecanismosDetalle.value;
  }
...
const gruposDisponibles = computed(() => {
  ...
  if (selectedDepartamentos.value.length === 0 && 
    ...
    console.log(`- Sin filtros previos, mostrando todos: ${grupos.value.length}`);
    return grupos.value;
  }
...
// Actualizar onDepartamentosChange para limpiar mecanismos y grupos
const onDepartamentosChange = () => {
  ...
  if (selectedDepartamentos.value.length > 0) {
    ...
    selectedMunicipios.value = selectedMunicipios.value.filter(codMun => {
      const municipio = municipiosList.value.find(m => String(m.cod_municipio) === codMun);
...
// Actualizar onTerritorialesChange para limpiar mecanismos y grupos
...
const territorialesDisponibles = computed(() => {
  ...
  if (selectedDepartamentos.value.length === 0) {
    console.log('- Sin filtro de departamentos, mostrando todas las territoriales');
...
// 4. MUNICIPIOS FILTRADOS FINAL CORREGIDO
const municipiosFiltrados = computed(() => {
  ...
  if (useCodigosMunicipioFilter.value && codigosValidados.value.length > 0) {
    const result = municipiosList.value.filter(m => 
  ...
  // Iniciar con todos los municipios
  let result = [...municipiosList.value];
  console.log(`- Total inicial: ${result.length} municipios`);
  ...
  if (selectedDepartamentos.value.length > 0) {
    ...
    if (result.length === 0 && municipiosList.value.length > 0) {
      ...
      console.log('  Tipo de municipio.cod_depto:', typeof municipiosList.value[0].cod_depto);
    }
  }
...
// 6. FUNCIÓN PARA DEBUGGING DETALLADO DE FILTROS
    ...
    if (!searchDepartamentos.value) {
        console.log(`- Sin búsqueda, mostrando todos: ${departamentos.value.length}`);
        return departamentos.value;
    }
    
    const search = searchDepartamentos.value.toLowerCase();
    const result = departamentos.value.filter(depto => 
        depto.nom_depto.toLowerCase().includes(search)
    );
    console.log(`- Con búsqueda "${search}": ${result.length} resultados`);
    return result;
    });
    ...
    // Municipios filtrados final
    // Corrección del municipiosFiltrados computed property
    ...
    const selectAllDepartamentos = () => {
      selectedDepartamentos.value = departamentos.value.map(d => d.cod_depto.toString());
    ...
    const onCodigosManualesChange = () => {
      ...
      // Validar códigos contra la lista de municipios
      const validos = [];
      const invalidos = [];
    ...
    selectedTerritoriales.value = [];
    selectedMecanismosGenerales.value = [];
    selectedMecanismosDetalle.value = []; // Nuevo
    selectedGrupos.value = [];
    ...
    const filtros = {
        ...
        selectedMecanismosDetalle: selectedMecanismosDetalle.value, // Nuevo
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            selectedMecanismosDetalle.value = filtros.selectedMecanismosDetalle || []; // Nuevo
