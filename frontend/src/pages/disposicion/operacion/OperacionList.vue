  ...
  <div class="productos-page">
    ...
    <main class="content-section">
      ...
      <div v-if="loading" class="loading-container">
        ...
        <span>Cargando municipios con operación...</span>
      </div>
      ...
      <div v-else-if="!municipiosFiltered.length" class="empty-container">
        ...
        <p>No se encontraron municipios con operación con los filtros aplicados</p>
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="municipio in paginatedMunicipios" :key="municipio.cod_municipio">
                ...
                <td class="text-center">
                  <span class="badge badge-evaluado">{{ municipio.evaluados || 0 }}</span>
                </td>
                <td class="text-center">
                  <span class="badge badge-aprobado">{{ municipio.aprobados || 0 }}</span>
                </td>
                <td class="text-center">
...
export default defineComponent({
  ...
  setup() {
    ...
    const municipiosConOperacion = ref<any[]>([])
    const todosLosMunicipiosConOperacion = ref<any[]>([])
    ...
    const municipiosFiltradosBase = computed(() => {
      let result = [...todosLosMunicipiosConOperacion.value]
    ...
    const departamentosDisponibles = computed(() => {
      let municipiosBase = [...todosLosMunicipiosConOperacion.value]
    ...
    const filtrosActivos = computed(() => {
      ...
      if (filters.value.municipio) {
        const mun = todosLosMunicipiosConOperacion.value.find(m => m.cod_municipio?.toString() === filters.value.municipio.toString())
        activos.push({ tipo: 'municipio', valor: mun?.nom_municipio || filters.value.municipio })
      }
      return activos
    })

    const totalMunicipiosBase = computed(() => todosLosMunicipiosConOperacion.value.length)
    ...
    const loadInitialData = async () => {
      try {
        ...
        const [deptosData, territorialesData, municipiosConOperData] = await Promise.allSettled([
          fetchAllPaginated(`${API_URL}/preoperacion/departamentos/`),
          fetchAllPaginated(`${API_URL}/preoperacion/territoriales/`),
          fetchAllPaginated(`${API_URL}/app/api/operacion-arbol/municipios_con_operacion/`)
        ])
        ...
        if (municipiosConOperData.status === 'fulfilled') {
          todosLosMunicipiosConOperacion.value = municipiosConOperData.value
          municipiosConOperacion.value = municipiosConOperData.value
        }
    ...
    const verDetalle = async (municipio: any) => {
      try {
        ...
        const response = await fetch(`${API_URL}/app/api/mecanismos-operacion/${municipio.cod_municipio}/`, {
          headers: {
    ...
    const exportarDatos = () => {
      try {
        const headers = ['Código', 'Municipio', 'Departamento', 'Territorial', 'Directorios', 'Archivos', 'Pendientes', 'Evaluados', 'Aprobados', 'Tamaño']
        ...
        link.href = url
        link.download = `operacion_${format(new Date(), 'yyyyMMdd_HHmmss')}.csv`
        link.click()
...
.modal-subtitle-mec strong { color: #8B4513; }
.loading-mecanismos {
  display: flex;
  align-items: center;
  justify-content: center;
...
.mecanismo-option:hover {
  border-color: #A0522D;
  background: linear-gradient(135deg, #fef7f0, #fdebd0);
  transform: translateX(4px);
}
.mecanismo-option:hover .material-icons { color: #A0522D; }
.mecanismo-info {
  display: flex;
  flex-direction: column;
  gap: 0.25rem;
