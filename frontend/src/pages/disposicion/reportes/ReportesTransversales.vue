<template>
  <div class="reportes-transversales-page">
    <!-- Cabecera -->
    <div class="page-header">
      <div class="header-content">
        <h1>Reportes Transversales</h1>
...
export default defineComponent({
  ...
  setup() {
    ...
    // Opciones de generación - Solo reportes individuales para transversales
    ...
    const generarReportes = async () => {
      ...
      try {
        loading.value = true;
        loadingMessage.value = 'Generando reportes Transversales...';
        ...
        // ✅ ÚNICA API DE TRANSVERSALES: Solo para generar reportes
        const response = await fetch(`${API_URL}/app/api/generar-reportes-transversal/`, {
          method: 'POST',
          headers: {
        ...
        link.href = url;
        link.download = `reportes_transversales_${new Date().toISOString().split('T')[0]}.zip`;
    ...
    const exportarFiltros = () => {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `filtros_transversales_${new Date().toISOString().split('T')[0]}.json`;
      link.click();
