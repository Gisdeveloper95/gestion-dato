<template>
  <div class="reportes-postoperacion-page">
        ...
        <div class="quick-search-section">
          ...
          <div class="search-input-container">
            ...
            <input 
              ...
              placeholder="🔍 Buscar por código (ej: 17174) o nombre de municipio (ej: CHINCHINA )..."
              class="search-input"
            />
            <button 
    ...
    <div v-if="municipiosFiltrados.length > 0" class="results-section">
      ...
      <div class="results-toolbar">
        ...
        <div class="toolbar-right">
          <button
            ...
            class="btn-success"
            :disabled="loading || (!generarReportesIndividuales && !generarReporteResumen) || !algunaEtapaSeleccionada"
          >
      ...
      <!-- Filtro de Etapas - HORIZONTAL -->
      <div class="etapas-filter-horizontal">
        <div class="etapas-label">
          <i class="material-icons">folder_special</i>
          <span>Etapas ({{ etapasSeleccionadas.length }}/{{ etapasPostOperacion.length }})</span>
          <button @click="seleccionarTodasEtapas" class="btn-mini-action" :disabled="todasEtapasSeleccionadas">Todas</button>
          <button @click="deseleccionarTodasEtapas" class="btn-mini-action" :disabled="!algunaEtapaSeleccionada">Ninguna</button>
        </div>
        <div class="etapas-chips">
          <label
            v-for="etapa in etapasPostOperacion"
            :key="etapa.prefijo"
            class="etapa-chip"
            :class="{ 'chip-selected': etapa.seleccionado }"
          >
            <input type="checkbox" v-model="etapa.seleccionado" />
            <span>{{ etapa.nombre }}</span>
          </label>
        </div>
      </div>
      <div v-if="!algunaEtapaSeleccionada" class="etapas-warning-inline">
        <i class="material-icons">warning</i>
        <span>Seleccione al menos una etapa</span>
      </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    // ✅ CORRIGIENDO: Datos maestros usando APIs CORRECTAS (preoperacion para datos)
    ...
    // ✅ CORRIGIENDO: Todos los filtros como preoperacion
    ...
    // ============ FILTRO DE ETAPAS POST-OPERACIÓN ============
    // Definición de las etapas operativas con sus prefijos y nombres descriptivos
    const etapasPostOperacion = ref([
      { prefijo: '01', nombre: '01 - Aprobación Económica', seleccionado: true },
      { prefijo: '02', nombre: '02 - Componente Social', seleccionado: true },
      { prefijo: '03', nombre: '03 - Pre-Cierre', seleccionado: true },
      { prefijo: '04', nombre: '04 - Resolución Inscripción', seleccionado: true },
      { prefijo: '05', nombre: '05 - Productos Catastrales', seleccionado: true },
      { prefijo: '06', nombre: '06 - Acta Entrega Base Catastral', seleccionado: true },
      { prefijo: '07', nombre: '07 - Acta Entrega Productos Catastrales', seleccionado: true },
      { prefijo: '08', nombre: '08 - Informe Plan Calidad', seleccionado: true },
      { prefijo: '09', nombre: '09 - Documentos Anexos', seleccionado: true },
      { prefijo: '10', nombre: '10 - Reporte Calidad Base Catastral', seleccionado: true },
      { prefijo: '11', nombre: '11 - Correspondencia', seleccionado: true },
      { prefijo: '12', nombre: '12 - Asistencia', seleccionado: true }
    ]);

    // Computed para obtener etapas seleccionadas
    const etapasSeleccionadas = computed(() => {
      return etapasPostOperacion.value
        .filter(e => e.seleccionado)
        .map(e => e.prefijo);
    });

    // Métodos para el filtro de etapas
    const seleccionarTodasEtapas = () => {
      etapasPostOperacion.value.forEach(e => e.seleccionado = true);
    };

    const deseleccionarTodasEtapas = () => {
      etapasPostOperacion.value.forEach(e => e.seleccionado = false);
    };

    const todasEtapasSeleccionadas = computed(() => {
      return etapasPostOperacion.value.every(e => e.seleccionado);
    });

    const algunaEtapaSeleccionada = computed(() => {
      return etapasPostOperacion.value.some(e => e.seleccionado);
    });
    ...
    const busquedaInmediataMunicipios = () => {
      console.log('🔍 Búsqueda inmediata de municipios (POST-OPERACION):', busquedaMunicipios.value);
    ...
    // ✅ CORRIGIENDO: APIs de PREOPERACION para búsqueda
    const realizarBusquedaMunicipios = async () => {
      try {
        buscandoMunicipios.value = true;
        const terminoBusqueda = busquedaMunicipios.value.trim();
        console.log('🔍 Término de búsqueda (POST-OPERACION - usando APIs preoperacion):', terminoBusqueda);
        ...
        // Estrategia múltiple de búsqueda usando APIs de PREOPERACION (las que SÍ existen)
        let municipiosEncontrados: any[] = [];
        
        // 1. Búsqueda por código exacto
        if (/^\d+$/.test(terminoBusqueda)) {
          console.log('🔍 Búsqueda por código (usando preoperacion):', terminoBusqueda);
          try {
        ...
        try {
          console.log('🔍 Búsqueda general con search (usando preoperacion):', terminoBusqueda);
          const municipiosBusquedaResult = await cargarTodosLosDatos(`${API_URL}/preoperacion/municipios/`, {
            search: terminoBusqueda
          });
          if (municipiosBusquedaResult.length > 0) {
            municipiosEncontrados = [...municipiosEncontrados, ...municipiosBusquedaResult];
          }
        } catch (error) {
        ...
        console.log(`✅ Búsqueda completada (POST-OPERACION): ${municipiosBusqueda.value.length} municipios encontrados`);
        
      } catch (error) {
        console.error('❌ Error en búsqueda de municipios (POST-OPERACION):', error);
    ...
    const limpiarBusquedaMunicipios = () => {
      console.log('🧹 Limpiando búsqueda de municipios (POST-OPERACION)');
    ...
    // ✅ CORRIGIENDO: Cargar datos iniciales usando APIs CORRECTAS (preoperacion)
    const loadInitialData = async () => {
      try {
        loading.value = true;
        loadingMessage.value = 'Cargando datos maestros...';
        
        console.log('🔄 Iniciando carga de datos (POST-OPERACION - usando APIs preoperacion)...');
        
        // ✅ ENDPOINTS CORRECTOS: PREOPERACION para datos maestros (como en el original)
        ...
        municipiosList.value = municipiosList.value.map(m => ({
          ...
          // Asegurar que estos valores existen
        ...
        // Debugging
        console.log('📊 DATOS CARGADOS (POST-OPERACION usando preoperacion APIs):');
        ...
        console.error('❌ Error cargando datos (POST-OPERACION):', error);
    ...
    // ✅ TODOS LOS COMPUTED PROPERTIES COMO EN PREOPERACION (copiados exactos)
    ...
    const filteredMunicipios = computed(() => {
      ...
      if (selectedDepartamentos.value.length > 0) {
        result = result.filter(m => {
          return selectedDepartamentos.value.includes(String(m.cod_depto));
        });
      }
      
      if (selectedTerritoriales.value.length > 0) {
        result = result.filter(m => {
          return m.nom_territorial && selectedTerritoriales.value.includes(m.nom_territorial);
        });
      }
    ...
    const filteredDepartamentos = computed(() => {
      if (!searchDepartamentos.value) {
        ...
        return departamentos.value.filter(d => deptosIds.has(String(d.cod_depto)));
      }
    ...
    const filteredTerritoriales = computed(() => {
      ...
      return territorialesDisponibles.value.filter(terr => 
    ...
    // Manejo del filtro especial por códigos (igual que preoperacion)
    ...
    // Manejo de archivos (igual que preoperacion)
    ...
    const clearAllFilters = () => {
      ...
      // Limpiar la búsqueda de municipios
    ...
    // ✅ GENERAR REPORTES - Solo la API de generación es postoperacion
    const generarReportes = async () => {
      ...
      try {
        ...
        const reportOptions = {
          ...
          etapas_filtro: etapasSeleccionadas.value  // Filtro de etapas POST
        };
        
        // ✅ ÚNICA API DE POSTOPERACION: Solo para generar reportes
        const response = await fetch(`${API_URL}/postoperacion/reportes/excel/`, {
          method: 'POST',
          headers: {
        ...
        console.error('Error generando reportes (POST-OPERACION):', error);
    ...
    // Watcher: Actualizar filtros cuando cambie la búsqueda
    ...
    return {
      ...
      etapasPostOperacion,
      etapasSeleccionadas,
      seleccionarTodasEtapas,
      deseleccionarTodasEtapas,
      todasEtapasSeleccionadas,
      algunaEtapaSeleccionada,
    };
  }
});
</script>
...
/* ============ NUEVA BARRA DE HERRAMIENTAS HORIZONTAL ============ */
.results-toolbar {
  display: flex;
  justify-content: space-between;
  align-items: center;
...
.etapa-chip input[type="checkbox"] {
  width: 12px;
  height: 12px;
  accent-color: #3b82f6;
  cursor: pointer;
...
/* ============ RESPONSIVE PARA FILTRO HORIZONTAL ============ */
@media (max-width: 768px) {
  .results-toolbar {
    flex-direction: column;
    gap: 0.75rem;
