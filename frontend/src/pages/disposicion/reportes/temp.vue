<template>
  <div class="reportes-operacion-page">
    <!-- Cabecera -->
      ...
      <div class="results-header">
        ...
        <div class="generation-options">
          ...
          <button 
            ...
            class="btn-success"
            :disabled="loading || !generarReportesIndividuales"
          >
...
export default defineComponent({
  ...
  setup() {
    ...
    // Computed properties (igual que en ReportesPostoperacion.vue)
    ...
    const municipiosFiltrados = computed(() => {
      ...
      // Si está usando el filtro por códigos específicos
      ...
      // Si NO hay municipios específicamente seleccionados
    ...
    const generarReportes = async () => {
      ...
      try {
        loading.value = true;
        loadingMessage.value = 'Generando reportes de operación...';
        ...
        const response = await fetch(`${API_URL}/app/api/generar-reportes-operacion/`, {
          method: 'POST',
          headers: {
            'Authorization': `Token ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json'
        ...
        link.href = url;
        link.download = `reportes_operacion_${new Date().toISOString().split('T')[0]}.zip`;
    ...
    const exportarFiltros = () => {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `filtros_operacion_${new Date().toISOString().split('T')[0]}.json`;
      link.click();
...
<style scoped>
/* 🎨 ESTILOS EN TONOS NEGROS/GRISES PARA OPERACIÓN - IGUAL QUE POSTOPERACION */
.reportes-operacion-page {
  display: flex;
