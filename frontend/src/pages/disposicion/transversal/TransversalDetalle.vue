<template>
  <div class="transversal-detalle-page">
    <!-- Header -->
    <header class="page-header">
      <div class="header-content">
        ...
        <div class="header-info">
          <h1>Transversal - {{ municipioName }}</h1>
    ...
    <main v-else class="main-content">
      ...
      <div class="tree-container">
        <div v-if="displayData.length === 0" class="empty-state">
          ...
          <p v-else>No se encontraron directorios transversales</p>
...
const linuxToWindowsPath = (linuxPath: string | null | undefined): string => {
  ...
  // Patron: /mnt/repositorio/ -> \\repositorio\DirGesCat\
...
const goBack = () => {
  ...
  if (window.history.length > 1) {
    ...
    router.push('/disposicion/transversal')
  }
}
...
const loadData = async () => {
  ...
  if (!codMpio) {
    error.value = 'Codigo de municipio no proporcionado'
    loading.value = false
    return
  }
  ...
  try {
    ...
    // Usar endpoint de transversal-arbol
    const response = await api.get('/app/api/transversal-arbol/arbol_realtime/', {
      params
    })
...
const previewFile = () => {
  ...
  // Para PDFs e imagenes, abrir en nueva pestana usando fetch + blob
...
const openFileInNewTab = async (ruta: string, token: string | null) => {
  try {
    ...
    // Limpiar URL despues de un tiempo
...
const downloadFile = async () => {
  ...
  if (downloading.value) {
    alert('Ya hay una descarga en curso')
    return
  }

  const token = localStorage.getItem('token')
  const fileName = selectedFile.value?.nombre || 'archivo'
  const ext = selectedFile.value?.extension?.toLowerCase() || ''

  downloading.value = true

  try {
    // Usar endpoint ver_pdf para todos los archivos (maneja multiples tipos)
    let downloadUrl = `${API_URL}/preoperacion/ver_pdf/?ruta=${encodeURIComponent(rutaAbsoluta)}&download=true`

    // Para archivos no-PDF, usar descargar_archivo si existe
    if (ext !== '.pdf') {
      downloadUrl = `${API_URL}/preoperacion/descargar_archivo/?ruta=${encodeURIComponent(rutaAbsoluta)}`
    }
...
<style scoped>
.transversal-detalle-page {
  min-height: 100vh;
  background: #f5f7fa;
}

/* Header - Color verde para Transversal */
.page-header {
...
/* Color por nivel - Tonos verdes */
.nivel-0 > .node-header .folder-icon { color: #1b5e20; }
.nivel-1 > .node-header .folder-icon { color: #2e7d32; }
.nivel-2 > .node-header .folder-icon { color: #388e3c; }
.nivel-3 > .node-header .folder-icon { color: #43a047; }
.nivel-4 > .node-header .folder-icon { color: #4caf50; }
.nivel-5 > .node-header .folder-icon { color: #66bb6a; }
.nivel-6 > .node-header .folder-icon { color: #81c784; }
.nivel-7 > .node-header .folder-icon { color: #a5d6a7; }
.nivel-8 > .node-header .folder-icon { color: #c8e6c9; }
.nivel-9 > .node-header .folder-icon { color: #e8f5e9; }
...
.btn-download-dir:hover:not(:disabled) {
  background: #0d7d73;
  transform: scale(1.1);
}

.btn-download-dir:disabled {
  opacity: 0.6;
  cursor: wait;
}
