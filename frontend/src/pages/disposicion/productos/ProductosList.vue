          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="municipio in paginatedMunicipios" :key="municipio.cod_municipio">
                ...
                <td class="text-center">
                  <span class="badge badge-primary">{{ municipio.evaluados || 0 }}</span>
                </td>
                <td class="text-center">
                ...
                <td>
                  <div class="actions">
                    <button @click="verDetalle(municipio)" class="action-btn view" title="Ver detalles">
                    ...
                    <button
                      ...
                      :disabled="downloading[municipio.cod_municipio]"
                      title="Descargar productos"
                    >
                      <i class="material-icons">
                        {{ downloading[municipio.cod_municipio] ? 'hourglass_empty' : 'download' }}
                      </i>
                    </button>
                  </div>
                </td>
    ...
    <div v-if="showMecanismoModal" class="modal-overlay" @click.self="closeMecanismoModal">
      <div class="modal-mecanismo">
        <div class="modal-header">
          ...
          <button @click="closeMecanismoModal" class="modal-close">
            <i class="material-icons">close</i>
          </button>
        </div>
        ...
        <div class="modal-footer">
          <button @click="closeMecanismoModal" class="btn-cancel">Cancelar</button>
        </div>
      </div>
    </div>
  </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    // Datos base (NUNCA se modifican, solo se usan para calcular opciones)
    const departamentos = ref<any[]>([])
    const territoriales = ref<any[]>([])
    const municipiosList = ref<any[]>([])
    const municipiosConProductos = ref<any[]>([])
    const todosLosMunicipiosConProductos = ref<any[]>([]) // ✅ BASE COMPLETA
    
    // ✅ FILTROS SIN AUTO-LIMPIEZA
    const filters = ref({
      departamento: '',
      municipio: '',
      territorial: ''
    ...
    const canDownload = computed(() => {
      ...
      return isSuperAdmin || isAdmin || isProfesional
    })
    ...
    const municipiosFiltradosBase = computed(() => {
      let result = [...todosLosMunicipiosConProductos.value]

      // 🔒 PRIMERO: Si es profesional, filtrar solo municipios asignados
    ...
    const departamentosDisponibles = computed(() => {
      // Partir de municipios filtrados por otros filtros (NO por departamento)
      let municipiosBase = [...todosLosMunicipiosConProductos.value]
      ...
      // Retornar solo departamentos que tienen municipios disponibles
    ...
    const territorialesDisponibles = computed(() => {
      // Partir de municipios filtrados por otros filtros (NO por territorial)
      let municipiosBase = [...todosLosMunicipiosConProductos.value]
      
      // Aplicar filtros EXCEPTO territorial
      if (filters.value.departamento) {
        municipiosBase = municipiosBase.filter(m => 
          m.cod_depto?.toString() === filters.value.departamento.toString()
        )
      ...
      // Contar municipios por territorial
      ...
      // Retornar solo territoriales que tienen municipios disponibles
      const territorialesConMunicipios = Array.from(terrCounts.keys())
        .map(nom => ({
          nom_territorial: nom,
      ...
      return territorialesConMunicipios
    })
    
    // ✅ 5. CALCULAR OPCIONES DISPONIBLES PARA MUNICIPIOS ESPECÍFICOS
    const municipiosParaSelector = computed(() => {
      // Partir de municipios filtrados por otros filtros (NO por municipio)
      let municipiosBase = [...todosLosMunicipiosConProductos.value]
      
      // Aplicar filtros EXCEPTO municipio específico
      if (filters.value.departamento) {
        municipiosBase = municipiosBase.filter(m => 
          m.cod_depto?.toString() === filters.value.departamento.toString()
        )
    ...
    const filtrosActivos = computed(() => {
      ...
      if (filters.value.municipio) {
        const mun = todosLosMunicipiosConProductos.value.find(m => 
          m.cod_municipio?.toString() === filters.value.municipio.toString()
        )
        activos.push({
          tipo: 'municipio',
    ...
    const totalMunicipiosBase = computed(() => {
      // Para profesionales, el total base es solo sus municipios asignados
      if (userPermissions.value.isProfesional && !userPermissions.value.isAnyAdmin) {
        const asignados = municipiosAsignados.value
        if (asignados.length > 0) {
          return todosLosMunicipiosConProductos.value.filter(m =>
            asignados.includes(String(m.cod_municipio))
          ).length
        }
      }
      return todosLosMunicipiosConProductos.value.length
    })
    ...
    const municipiosSorted = computed(() => {
      ...
      sorted.sort((a, b) => {
        ...
        if (['cod_municipio', 'total_productos', 'evaluados', 'aprobados', 'pendientes'].includes(sortField.value)) {
          aVal = Number(aVal) || 0
          bVal = Number(bVal) || 0
        }
    ...
    const loadInitialData = async () => {
      try {
        ...
        const [deptosData, territorialesData, municipiosData, municipiosConProductosData] = await Promise.allSettled([
          fetchAllPaginated(`${API_URL}/preoperacion/departamentos/`),
          fetchAllPaginated(`${API_URL}/preoperacion/territoriales/`),
          fetchAllPaginated(`${API_URL}/preoperacion/municipios/`),
          fetchAllPaginated(`${API_URL}/postoperacion/municipios-con-productos/`), // ✅ SIN FILTROS
        ])
        ...
        // ✅ GUARDAR DATOS BASE COMPLETOS
        if (municipiosConProductosData.status === 'fulfilled') {
          todosLosMunicipiosConProductos.value = municipiosConProductosData.value
          municipiosConProductos.value = municipiosConProductosData.value
        }
    ...
    // ✅ CAMBIO IMPORTANTE: SIN AUTO-LIMPIEZA
    const onFilterChange = () => {
      currentPage.value = 1
      // NO llamamos loadMunicipiosConProductos() porque ya tenemos todos los datos
      // Los filtros se aplican a través de computed properties
    }
    ...
    // Verificar mecanismos y mostrar modal si hay múltiples
    const verDetalle = async (municipio: any) => {
      try {
        loadingMecanismos.value = true
        municipioSeleccionado.value = municipio

        // Consultar mecanismos del municipio
        const response = await fetch(`${API_URL}/postoperacion/mecanismos/${municipio.cod_municipio}/`, {
          headers: {
        ...
        if (!response.ok) {
          // Si falla, navegar directamente sin filtro
          router.push(`/disposicion-informacion/productos/${municipio.cod_municipio}`)
          return
        }
        ...
        if (data.success && data.tiene_multiples) {
          // Tiene múltiples mecanismos, mostrar modal
          mecanismosDisponibles.value = data.mecanismos
          showMecanismoModal.value = true
        } else {
          // Solo tiene un mecanismo o ninguno, navegar directamente
          if (data.mecanismos && data.mecanismos.length === 1) {
            // Navegar con el mecanismo seleccionado
            router.push({
              path: `/disposicion-informacion/productos/${municipio.cod_municipio}`,
              query: { mecanismo: data.mecanismos[0].codigo }
            })
          } else {
            // Sin mecanismos específicos, navegar sin filtro
            router.push(`/disposicion-informacion/productos/${municipio.cod_municipio}`)
          }
        }
      } catch (error) {
    ...
    const seleccionarMecanismo = (mecanismo: any) => {
      if (municipioSeleccionado.value) {
        router.push({
          path: `/disposicion-informacion/productos/${municipioSeleccionado.value.cod_municipio}`,
          query: { mecanismo: mecanismo.codigo }
        })
        closeMecanismoModal()
      }
    }
    ...
    // 🔧 REEMPLAZAR la función descargarProductos en ProductosList.vue

    const descargarProductos = async (municipio: any) => {
      if (!canDownload.value) {
        showNotification('No tienes permisos para descargar archivos', 'error')
        return
      }
      
      try {
        downloading.value[municipio.cod_municipio] = true
        showNotification(`Generando reporte para ${municipio.nom_municipio}...`, 'info')
        
        // ✅ USAR LA MISMA API QUE ReportesPostoperacion.vue
        ...
        showNotification(`Productos de ${municipio.nom_municipio} descargados`, 'success')
      } catch (error) {
        console.error('Error descargando productos:', error)
        showNotification(`Error al descargar productos de ${municipio.nom_municipio}`, 'error')
      } finally {
        downloading.value[municipio.cod_municipio] = false
      }
    }
    ...
    const exportarDatos = () => {
      ...
      try {
        const headers = ['Código', 'Municipio', 'Departamento', 'Territorial', 'Total', 'Evaluados', 'Aprobados', 'Pendientes']
        ...
        const link = document.createElement('a')
        link.href = url
        link.download = `productos_municipios_${format(new Date(), 'yyyyMMdd_HHmmss')}.csv`
    ...
    return {
      ...
      // ✅ NUEVOS COMPUTED PARA FILTROS ACUMULATIVOS
...
/* ✅ NUEVO: CONTENEDORES DE FILTROS CON LABELS */
.filter-container {
  display: flex;
  flex-direction: column;
  gap: 0.5rem;
...
.mecanismo-option:hover .material-icons {
  color: #3b82f6;
}
